...
const SidebarMenuItems = ({ mainmenu, setMainMenu, sidebartoogle, setNavActive, activeClass }) => {
  ...
  const toggletNavActive = (item) => {
      ...
      mainmenu.map((a) => {
        a.Items.filter((Items) => {
          ...
          Items.children.forEach((b) => {
            if (Items.children.includes(item)) b.active = false;
    ...
    if (item.active) {
      setTimeout(() => {
        const el = document.querySelector(".sidebar-list.active");
      ...
      {mainmenu.map((Item, i) => (
        <Fragment key={i}>
          ...
          {Item.Items.map((menuItem, i) => (
            <li
              className={`sidebar-list ${menuItem.active ? "active" : ""}`}
              key={i}
            >
              {menuItem.type === "sub" ? (
                <a
                  ...
                  {menuItem.children.map((childrenItem, index) => (
                    <li key={index}>
                      ...
                      {childrenItem.type === "link" ? (
                        <Link
                          to={getLinkPath(childrenItem.path)}
                          className={`${CurrentPath.includes(childrenItem?.title?.toLowerCase()) ? "active" : ""}`}
                      ...
                      {childrenItem.children ? (
                        <ul
                          ...
                          style={
                            CurrentPath.includes(childrenItem?.title?.toLowerCase()) ||
                            childrenItem.active
                              ? { display: "block" }
                              : { display: "none" }
                          }
                          ...
                          {childrenItem.children.map((childrenSubItem, key) => (
                            <li key={key}>
                              {childrenSubItem.type === "link" ? (
                                <Link
                                  to={getLinkPath(childrenSubItem.path)}
