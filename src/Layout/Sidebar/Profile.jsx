...
const Profile = () => {
  const { isAuthenticated, avatar, user } = useSelector((state) => state.auth);
  const auth0_profile = JSON.parse(localStorage.getItem('auth0_profile'));
  const [profile, setProfile] = useState('');
  const [name, setName] = useState('');

  useEffect(() => {
    setProfile(avatar || localStorage.getItem('profileURL') || man);
    ...
    <Fragment>
      <div className='sidebar-user text-center'>
        ...
        <Link to={`${process.env.PUBLIC_URL}/app/users/userProfile`}>
          <Image attrImage={{ className: 'img-90 rounded-circle', src: isAuthenticated ? (auth0_profile?.picture || profile) : profile, alt: '' }} />
          <H6 attrH6={{ className: 'mt-3 f-14 f-w-600' }}>{isAuthenticated ? (user?.name || auth0_profile?.name || name) : name}</H6>
        </Link>
