...
const CustomizerProvider = (props) => {
  ...
  const layoutURL = safeLayout(localStorage.getItem('layout')) || (defaultLayoutObj && Object.keys(defaultLayoutObj).pop()) || 'Admin';
  const layoutValue = (defaultLayoutObj && Object.values(defaultLayoutObj).pop()) || 'compact-wrapper';

  useEffect(() => {
    const savedMixLayout = localStorage.getItem('mix_background_layout') || ConfigDB.data.color.mix_background_layout;

    setLayout(layoutValue);
    setSidebarIconType(localStorage.getItem('sidebar_icon_type') || ConfigDB.data.settings.sidebar.iconType);
    setMixBackgroundLayout(savedMixLayout);
    setMixLayout(savedMixLayout === 'light-only');
    ConfigDB.data.color.mix_background_layout = savedMixLayout;
    document.body.classList.remove('light-only', 'dark-only', 'dark-sidebar');
    document.body.classList.add(savedMixLayout);
    const matchedLayout = classes.find((item) => Object.keys(item).pop() === layoutURL);
    const resolvedSidebarLayout = matchedLayout ? Object.values(matchedLayout).pop() : layoutValue;
    localStorage.setItem('sidebar_layout', resolvedSidebarLayout);
    ConfigDB.data.settings.sidebar.type = resolvedSidebarLayout;
    setLayout(resolvedSidebarLayout);
