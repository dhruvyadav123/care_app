  ...
  useEffect(() => {
    if (isOpen && questionData) {
      const normalizedOptions = Array.isArray(questionData?.options)
        ? questionData.options.map(normalizeOption).filter(Boolean)
        : [];
                ...
                {typeUsesOptions(formData.type) ? (
                  <div className="d-flex flex-column">
                    {formData.options.map((option, index) => (
                      <Row key={`caregiver-edit-option-${index}`} className="g-1 align-items-center option-row">
                        <Col md="8">
                          <Input
